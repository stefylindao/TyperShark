...
/**
 * La clase Pirana hereda de la clase Objeto.
 * Crea objetos de tipo Pirana.
 * 
 * @version: 	0.1		24/08/2016
 * @author: 	Paul Estrada León, Stefany Lindao Rodríguez, Elizabeth Sánchez Villamar.
 */
public class Pirana extends Objeto implements Movimiento, Animacion, ProcesarLetra, Runnable {
	...
	/**
	 * Método que construye un objeto que representa a la piraña, 
	 * le da una posición X y Y deseada junto a una imagen animada. 
	 * @param: posicionX
	 * @param: posicionY
	 */
	public Pirana(Double posicionX, Double posicionY) {
		super("pirana", posicionX, posicionY, 1);
		super.setVivo(false);
		super.getImageView().setImage(new Image("pirana.gif"));
		super.getImageView().setLayoutX(posicionX);
		super.getImageView().setLayoutY(posicionY);
		super.getImageView().setFitWidth(100);
				...
				public void run () {
					if (palabraAsignada.equals(letrasIngresadas.toLowerCase())) {
						...
						if (matando == false) {
							matando = true;
							try {
								Usuario.quitarVidaDesdePirana();
							} catch (IOException e) {
							}
						} else {
							if (getImageView().getLayoutX() <= -100.0) {
								matar();
							} else {
								mover(-0.30,0.0);
							}
	 ...
	 * Se encarga de verificar si el caracter ingresado por teclado 
	 * concuerdan con el caracter que lleva el objeto.
	 * @param: letra
	 */
	...
	/**
	 * Método que coloca un nuev caracter aleatorio al objeto.
	...
	public void colocarPalabra () throws FileNotFoundException, IOException {
		ManejoArchivos lector = new ManejoArchivos("caracteres.txt");
